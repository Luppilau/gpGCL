            ...
            fn transform_logical_expr_op(&mut self, i: LogicalExprOp) -> LogicalExprOp {
                match i.op {
                    LogicalExprOpcode::GreaterThan => LogicalExprOp {
                        op: LogicalExprOpcode::LessThan,
                        left: i.right,
                        right: i.left,
                    },
                    LogicalExprOpcode::GreaterThanOrEq => LogicalExprOp {
                        op: LogicalExprOpcode::LessThanOrEq,
                        left: i.right,
                        right: i.left,
                    },

                    _ => grammar::transform::transform_logical_expr_op(self, i),
                }
            }
        }
...
fn execute(input: String) -> String {
    ...
    let respose = ExecutionResponse {
        ...
        errors: vec![std::str::from_utf8(&output.stderr).unwrap().to_string()],
    };
